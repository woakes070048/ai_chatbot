...
def format_html_email(
	...
	"""
	# Fix markdown list formatting, then convert to HTML
...
def _style_html_tables(html: str) -> str:
	...
	Markdown-to-HTML converters produce plain <table> tags without inline
	styles. Email clients and PDF renderers (wkhtmltopdf) ignore <style>
	blocks, so every element needs inline CSS.

	Uses (?=[> ]) lookahead on <th> and <td> to avoid matching <thead>,
	<tbody>, <tfoot> etc.
	"""
	...
	html = re.sub(
		r"<table(?![^>]*style=)([^>]*)>",
		r"<table style='border-collapse: collapse; width: 100%; margin-bottom: 15px;'\1>",
		html,
	)

	# Style <th> tags — use (?=[> ]) to avoid matching <thead>
	html = re.sub(
		r"<th(?=[> ])(?![^>]*style=)([^>]*)>",
		r"<th style='padding: 8px; border: 1px solid #ddd; background-color: #f5f5f5; text-align: left;'\1>",
		html,
	)

	# Style <td> tags — use (?=[> ]) to avoid matching <tbody> etc.
	html = re.sub(
		r"<td(?=[> ])(?![^>]*style=)([^>]*)>",
		r"<td style='padding: 8px; border: 1px solid #ddd;'\1>",
		html,
	)
...
<h2 style="margin: 0 0 5px; color: #2c3e50;">{report_name}</h2>
...
def _render_charts(tool_results: list | None, as_svg: bool = False) -> str:
	...
	for option in echart_options:
		if as_svg:
			...
			if svg:
				html_parts.append(
					f'<div style="margin: 20px 0; text-align: center;">{svg}</div>'
				)
		else:
